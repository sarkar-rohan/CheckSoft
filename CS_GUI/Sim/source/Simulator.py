...
#(self.currentState, event))
    ...
    def printStatus(self):
        print "Human %d: currentState = %s, content = %s" % \
...
class StorageEntity(Entity):
    ...
    def processEvent(self, event, extraArgs=None):
        ...
        if (self.currentState, event) in storageTransitionTable:
            ...
            print "Warning: illegal StorageEntity transition", \
                (self.currentState, event)
            nextState = self.currentState
...
class Simulator(object):
    ...
    def generateRandomEventSequence(self, nHuman, nStorage, objects,
                                    eventList, eventPDF, maxIter, maxHumans):
        eventSequence = []
        self.humanEntities = [HumanEntity(i + 1) for i in range(nHuman)]
        self.storageEntities = [StorageEntity(i + 1) for i in range(nStorage)]
        ...
        while humanActive and timeStep < maxIter:
            ...
            if event == HUMANENTER and countHumans < maxHumans :
                ...
                storage = self.storageEntities[human.auxiliaryId - 1]
...
def main(mode):
    if mode == 0:
        ...
        sim = Simulator()
        nHuman, nStorage = int(sys.argv[1]), int(sys.argv[2])/2
        # objects = ['Apple', 'Orange', 'Grape', 'Pear', 'Banana', 'Avocado',
        #            'Pineapple', 'Watermelon', 'Blueberry', 'Coconut']
        objects = Counter({0: 100000, 1: 200000, 2: 100000, 3: 100000, 4: 200000, 5: 100000, 6: 100000, 7: 200000, 8: 100000, 9: 100000})
        eventList = [HUMANENTER, HUMANEXIT, HANDSIN, HANDSOUT, NOISE]
        eventPDF = np.array([0.1, 0.2, 0.5, 0.5, 0.2])
        eventPDF /= np.sum(eventPDF)
        maxHumans = int(sys.argv[2])/2-5
        maxIter = 1000000
        countNoiseProb=float(sys.argv[3])/10.0
        frames=10
        contentNoiseProb=float(sys.argv[4])/10.0
        eventSequence = sim.generateRandomEventSequence(
            nHuman, nStorage, objects, eventList, eventPDF, maxIter, maxHumans)
